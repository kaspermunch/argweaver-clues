...
for clues_file_name in clues_file_names:
    chrom, start, end, pop, pos, chain = re.search(r'(\d+)_(\d+)_(\d+)_([^_]+)_(\d+)_(\d+).h5', clues_file_name).groups()
    h5 = h5py.File(os.path.join(steps_dir, clues_file_name), 'r')
    log_likelihood_ratio = h5['logLikelihoodRatios'][h5.attrs['iHat'], h5.attrs['jHat']]
    selection_coef = h5.attrs['sHat']

    print(chrom, start, end, pop, pos, chain, log_likelihood_ratio, selection_coef, sep='\t', file=output_file)
